...
plt.bar(x + x_offs * bar_width, dfs[0]["native"]["average_host"].values,
        bottom=dfs[0]["native"]["transfer+kernel"].values, color=upper_bar_colors[0], **bar_args)
plt.errorbar(x + x_offs * bar_width, dfs[0]["native"]["average"].values,
                yerr=dfs[0]["native"]["stddev"].values, **errorbar_args)
x_offs += 1
...
for j, (bl, bh) in enumerate(zip(bars_low, bars_high)):
    plt.text(bl.get_x() + 0.03, bl.get_height() + bh.get_height() + 0.2,
                f" {proteus_overhead[j]:.1f}%", rotation=90, size=9)
plt.errorbar(x + x_offs * bar_width, dfs[0]["proteus"]["average"].values,
                yerr=dfs[0]["proteus"]["stddev"].values, **errorbar_args)
x_offs += 1
