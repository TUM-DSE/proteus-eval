...
df_proteus_u50      = pd.read_csv(f"../data/proteus/u50-fast-rosetta.csv", skipinitialspace=True)
df_proteus_u280     = pd.read_csv(f"../data/proteus/u280-fast-rosetta.csv", skipinitialspace=True)
df_proteus_u280_ddr = pd.read_csv(f"../data/proteus/u280-ddr-fast-rosetta.csv", skipinitialspace=True)
...
dfs = [df_u50_fast, df_u280_fast, df_u280_ddr_fast, df_proteus_u50, df_proteus_u280, df_proteus_u280_ddr]
df_labels = ["u50_fast", "u280_fast", "u280_ddr_fast", "u50_fast", "u280_fast", "u280_ddr_fast"]

df_app_name_vitis = dfs[0]["app_name"].copy()
df_app_name_rosetta = dfs[3]["app_name"].copy()
app_names_vitis = df_app_name_vitis.values 
app_names_rosetta = df_app_name_rosetta.values
# app_names = np.concatenate((app_names_vitis, app_names_rosetta))
# print(app_names)

### Vitis Accel Examples
for app in app_names_vitis:
    # skip wide_mem_rw
